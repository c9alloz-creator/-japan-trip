...
let edits   = loadJSON(STORAGE.edits, {});    // { activityId: { title, note, time, img, map } }
let added   = loadJSON(STORAGE.added, {});    // { bucketKey: [activity, ...] }
...
function effectiveList(baseItems, bucketKey) {
  return baseItems.filter((a) => !removed[a.id]).concat(added[bucketKey] || []);
}
...
function activityHTML(a, opts = {}) {
  ...
  const custom = edits[a.id] && edits[a.id].img;
...
function wireEvents() {
  ...
  document.getElementById("app").addEventListener("click", (e) => {
    ...
    if (x) {
      ...
      if (confirm("Remove your custom photo and go back to the default tile?")) {
        if (edits[id]) { delete edits[id].img; if (!Object.keys(edits[id]).length) delete edits[id]; }
    ...
    if (linkBtn) {
      ...
      const current = edits[id] && edits[id].img && !String(edits[id].img).startsWith("data:")
        ? edits[id].img : "";
    ...
    if (del) {
      ...
      if (id.startsWith("u-")) {
        ...
        for (const key of Object.keys(added)) {
          added[key] = added[key].filter((a) => a.id !== id);
          if (!added[key].length) delete added[key];
        }
      ...
      delete checks[id]; saveJSON(STORAGE.checks, checks);
      delete edits[id];  saveJSON(STORAGE.edits, edits);
      renderAll();
      return;
    }
  ...
  document.getElementById("photo-input").addEventListener("change", (e) => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        try {
          edits[id] = edits[id] || {};
          edits[id].img = dataURL;
          saveJSON(STORAGE.edits, edits);
          renderAll();
        } catch (err) {
          alert("This browser's photo storage is full — remove a few custom photos (✕) and try again.");
        }
      };
  ...
  document.getElementById("reset-edits").addEventListener("click", () => {
    if (!confirm("Restore the ORIGINAL plan on this device? This undoes all text edits, custom photos, map-link changes, and added or deleted activities. (Checkmarks are kept.)")) return;
  ...
  document.getElementById("reset-checks").addEventListener("click", () => {
    if (!confirm("Clear ALL checkmarks on this device?")) return;
    checks = {};
    saveJSON(STORAGE.checks, checks);
    renderAll();
  });
